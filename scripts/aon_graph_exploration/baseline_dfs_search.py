"""
Highly Optimized Branch and Bound Depth-First Search (DFS) on Activity-on-Node (AoN) Graph

This script performs an exhaustive DFS search to find paths that visit the maximum number
of countries within a 24-hour window, using branch and bound pruning with Minimum Travel Time (MTT).
"""
...
def calculate_upper_bound(current_countries: FrozenSet[str], current_arr_time: int, 
                         deadline: int, current_country: str, MTT: Dict[str, float]) -> int:
...
def format_path_details(path: List[Tuple], aon_country_map: Dict) -> str:
    ...
    for i, node in enumerate(path):
        ...
        lines.append(f"  Step {i+1}: {origin} → {dest} ({country_str}) | "
...
def save_checkpoint(paths_by_score, current_global_max, total_nodes, elapsed_time, 
                   aon_country_map, station_country_map):
    """
    Save a checkpoint of current progress to JSON file.
    Only saves paths with score >= 14 or score == current_global_max.
    """
    output_dir = SCRIPT_DIR / "results"
    output_dir.mkdir(exist_ok=True)
    checkpoint_file = output_dir / "live_checkpoint.json"
    
    # Build routes dictionary
    routes = {}
    for score, paths in paths_by_score.items():
        # Only save paths with score >= 14 or score == current_global_max
        if score >= 14 or score == current_global_max:
            ...
            for path in paths:
                ...
                route_list.append(path_steps)
            routes[str(score)] = route_list
...
def main():
    ...
    print("="*80)
    print("Branch and Bound DFS Search on AoN Graph")
    print("="*80)
    ...
    print(f"  ✓ Graph converted to dictionary format")
    
    # Perform DFS search with multiprocessing
    print("\n" + "="*80)
    print(f"Starting multiprocessed DFS search from {len(starting_nodes):,} starting nodes")
    print(f"{'='*80}\n")
    ...
    worker_args = [
        (start_node, shared_global_max, shared_lock)
    ...
    with ProcessPoolExecutor(
        ...
        # Submit all tasks
        future_to_node = {
            executor.submit(explore_start_node, args): args[0] 
            for args in worker_args
        }
        
        # Process results as they complete with progress bar
        with tqdm(total=len(starting_nodes), desc="Processing starting nodes", unit="node") as pbar:
            for future in as_completed(future_to_node):
                ...
                for num_countries, path in saved_paths:
                    ...
                    if num_countries > previous_global_max:
                        ...
                        print(f"\n{'='*80}")
                        print(f"🎉 NEW GLOBAL MAXIMUM: {num_countries} COUNTRIES!")
                        print(f"{'='*80}")
    ...
    if paths_by_score:
        ...
        if absolute_max < 14:
            ...
            paths_to_keep = {score: paths for score, paths in paths_by_score.items() if score >= 14}
            paths_by_score = paths_to_keep
    ...
    print(f"Global Maximum Countries: {global_max_countries}")
    print(f"Total Paths Saved: {total_paths}")
    if paths_by_score:
        print(f"Paths by Score: {dict((score, len(paths)) for score, paths in sorted(paths_by_score.items()))}")
    print("="*80)
    ...
    output_file = output_dir / "baseline_dfs_results.pickle"
