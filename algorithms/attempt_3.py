...
def label_setting_algorithm(V, Es, Et, country_map, R_func, bucket_func, MTT, record, compute_score):
    """
    Algorithm 6: Label-setting algorithm with time-budgeted UB pruning
    """
    Labels = {v: [] for v in V}
    Q = []
    best_label = None
    ...
    while Q:
        ...
        # --- ALGORITHM 6: TIME-BUDGETED UPPER BOUND ---
        U = R_func(v, bucket_func(t)) - C
        
        # Sort unvisited countries by their Minimum Transit Time (cheapest first)
...
def save_route_to_file(path, G, country_map, Es, Et, output_file="optimal_route.txt"):
    """
    Save the optimal route to a human-readable text file.
    
    Args:
        path: List of Label objects representing the route
        G: NetworkX graph with node and edge attributes
        country_map: Dictionary mapping nodes to countries
        Es: Scheduled edges dictionary
        Et: Transfer edges dictionary
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        for i, label in enumerate(path):
            ...
            f.write(f"  Time: {format_time_minutes(label.t)} (Elapsed: {time_elapsed:.1f} mins / {time_elapsed/60:.2f} hrs)\n")
            ...
            if len(countries_at_step) <= 10:
                ...
                f.write(f"  Countries: {', '.join(countries_at_step[:10])} ... (+{len(countries_at_step)-10} more)\n")
            ...
            if i > 0:
                ...
                # Check if it was a scheduled edge (flight/train)
                found_transport = False
                for dest, dep, arr in Es.get(prev_node, []):
                    if dest == node and dep == prev_time and arr == current_time:
                        # Find the edge type in the graph by checking time_pairs
                        edge_type = None
                        for u, v, key, data in G.edges(prev_node, keys=True, data=True):
                            if v == node:
                                edge_type = data.get('edge_type', 'unknown')
                                # Verify this edge has the matching time pair
                                time_pairs = data.get('time_pairs', [])
                                if (dep, arr) in time_pairs:
                                    break
                        transport_type = "FLIGHT" if edge_type == 'flight' else "TRAIN" if edge_type == 'train' else "SCHEDULED"
                        duration = arr - dep
                        f.write(f"  → Transport: {transport_type} from {prev_node} to {node}\n")
                        f.write(f"    Departure: {format_time_minutes(dep)}, Arrival: {format_time_minutes(arr)}\n")
                        f.write(f"    Duration: {duration:.1f} minutes ({duration/60:.2f} hours)\n")
                        found_transport = True
                        break
                
                # Check if it was a transfer edge (connection)
...
if __name__ == "__main__":
    ...
    # C. Precompute Heuristics (Alg 3 and Alg 5)
    ...
    print("[INFO] Searching for routes visiting maximum countries within 24 hours")
    ...
    best_path = label_setting_algorithm(
        ...
        MTT=precomputed_MTT,       # <--- Pass MTT here
    ...
    # F. Save route to file
    if best_path:
        output_filename = f"optimal_route_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
