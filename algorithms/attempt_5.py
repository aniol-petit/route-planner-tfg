...
def label_setting_algorithm_with_global_hard_floor(
    V,
    Es,
    Et,
    country_map,
    ...
    record is static (never updated during search).
    """
    ...
    print(
        ...
        f"Target record (static): {record}. Rollout freq: {rollout_freq}. GlobalMinTime: {GlobalMinTime}"
    )
    ...
    while Q:
        ...
        # --- GLOBAL HARD FLOOR PRUNING ---
        needed = (record + 1) - len(C)
        remaining_time = 1440 - (t - start_time)
        # If we need countries but lack the physical time (e.g., < 30 mins per country), kill it.
        if needed > 0 and remaining_time < (needed * GlobalMinTime):
            continue
        # ----------------------------------
        ...
        if rollout_freq and (iters % rollout_freq == 0):
            ...
            if best_label and len(best_label.C) > record:
                print(f"[ROLLOUT] Early stop: |best_label.C|={len(best_label.C)} > record={record}")
                break

        # Enforce 24-hour journey limit (1440 absolute minutes)
        if remaining_time < 0:
            continue

        # --- Upper bound computation (Time-Budgeted) ---
        ...
        # Record-based pruning (Using static record)
        if len(C) + UB <= record:
            continue
        ...
        # Early stopping if normal expansion beats the static record
        if len(C) > record:
            print(f"[SUCCESS] Early stop: |C|={len(C)} > record={record}")
            break
        ...
        for w, delta in Et.get(v, []):
            ...
            # As in Algorithm 9 pseudocode: transfers do not change C here
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run label-setting algorithm (Algorithm 9) with Global Hard Floor, Time-Budgeted UB + Monte-Carlo rollouts"
    )
    parser.add_argument(
        "--graph",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=40,
        help="Minimum realistic time to visit a country (in minutes, default: 40)",
    )
    ...
    print(f"\n[SETUP] Starting Algorithm 9 with target record: {current_record_to_beat} countries...")

    best_path = label_setting_algorithm_with_global_hard_floor(
        V=V,
        Es=Es,
        Et=Et,
        ...
        GlobalMinTime=args.global_min_time,
    )
    ...
    if best_path:
        output_filename = f"optimal_route_attempt5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
