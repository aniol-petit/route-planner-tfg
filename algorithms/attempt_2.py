...
# Attempt corresponding to Algorithm 4
...
if __name__ == "__main__":
    ...
    def compute_relaxed_reachability(V, Es, Et, country_map, bucket_func, bucket_size_mins=60):
        """
        Implements Algorithm 3: Relaxed reachability backpropagation.
        Precomputes the set of reachable countries from any node at any time bucket.
        """
        print("\n[INFO] Precomputing Relaxed Reachability (Algorithm 3)...")
        start_time = time.time()
        
        # 1. Find the dynamic range of time buckets present in the scheduled edges
        all_buckets = set()
        for v in V:
            for u, dep, arr in Es.get(v, []):
        ...
        # 3. Build relaxed adjacency lists: Adj(v, b)
    ...
    def my_R_func(v, b):
        """
        Reachability function using the precomputed Algorithm 3 table.
        """
        # If bucket exists in our precomputed bounds, return it.
        # Otherwise, assume only the node's own country is reachable.
        if b in precomputed_R[v]:
            return precomputed_R[v][b]
        
        # Fallback for out-of-bounds time
        c = country_map.get(v, 'Unknown')
