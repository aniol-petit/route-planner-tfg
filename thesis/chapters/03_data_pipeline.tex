...
\]
Let $t_{\textsl{prev}}$ denote the previously validated absolute timestamp and $M_{\textsl{current}}$ the current local minute value. The previous minute-of-day is:
\[
M_{\textsl{prev}} = t_{\textsl{prev}} \bmod 1440.
\]
A rollover flag is raised when the local clock wraps around:
\[
R =
\begin{cases}
1, & \text{if } M_{\textsl{current}} < M_{\textsl{prev}}, \\
0, & \text{otherwise}.
\end{cases}
\]
With day index $D = \lfloor t_{\textsl{prev}} / 1440 \rfloor$, the current absolute timestamp is computed as:
\[
t_{\textsl{abs}} = (D + R)\times 1440 + M_{\textsl{current}}.
\]
...
Because Europe spans multiple time zones, locally normalized $t_{\textsl{abs}}$ values are not directly comparable across borders. Without this step, the algorithm would perceive a train departing Madrid at 10:00 and a flight departing Lisbon at the same clock time as simultaneous events, when in reality the flight departs an hour earlier in absolute terms. A single unresolved timezone offset anywhere in the graph would silently corrupt every downstream routing decision that crossed that border.

\medskip

To ensure strict chronological consistency, every absolute timestamp was projected onto a UTC+0 reference axis. Let $\Delta_{\textsl{offset}}$ denote the local UTC offset in minutes. The synchronized value is:
\[
t_{\textsl{UTC}} = t_{\textsl{abs}} - \Delta_{\textsl{offset}}.
\]
Determining $\Delta_{\textsl{offset}}$ robustly across heterogeneous nodes required a cascading lookup strategy:

\begin{itemize}
  \item \textbf{Node-level overrides:} Explicit hardcoded mappings were applied first for exceptional nodes where country defaults are unreliable (e.g., Canary Islands $\Delta_{\textsl{offset}}=0$, Azores $\Delta_{\textsl{offset}}=-60$, and eastern Russian hubs such as Yekaterinburg $\Delta_{\textsl{offset}}=300$).
  ...
  \item \textbf{Continental default:} Remaining nodes fell back to the Central European baseline $\Delta_{\textsl{offset}}=60$.
\end{itemize}
...
\[
t_{\textsl{UTC}}^{\textsl{dep}} < t_{\textsl{UTC}}^{\textsl{arr}}.
\]
...
\begin{itemize}
  ...
  \item \textbf{GeoNames cross-referencing:} Querying a localized subset of the GeoNames database (see dataset in \cite{geonames}) to map normalized station strings to the most populous European city matching that name.
\end{itemize}
...
Although heuristic pruning removed most low-value nodes, identifying the most strategic international hubs across roughly 40 countries required additional geographic context. The pipeline therefore integrated Gemini 2.5 Pro as a constrained classifier rather than a routing engine. (see \cite{gemini2023} for information about the Gemini models family and their capabilities) National station lists were provided after heuristic filtering, and the model was prompted to return only internationally valuable hubs (e.g., cross-border high-speed access and strong air--rail interconnectivity). Outputs were constrained to structured JSON for direct downstream use. This hybrid stage combined deterministic topological filtering with LLM-based geographic prioritization, yielding a compact graph concentrated around high-value transfer hubs.
...
\begin{itemize}
  \item \textbf{Generator agent:} evaluated hub pairs within the same country, proposed plausible intra-city connections, and estimated transit durations (local transit plus walking). A hard cutoff removed proposals with $\tau_{\textsl{transfer}} > 240$ minutes.
